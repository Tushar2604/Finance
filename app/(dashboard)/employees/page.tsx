        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>All current and past employees.</CardDescription>
            </div>
                ...
                <TableBody>
                  {data?.data?.length === 0 ? (
                    ...
                    data?.data?.map((emp: any) => (
                      <TableRow key={emp._id} className="hover:bg-slate-50 transition-colors cursor-pointer">
                        <TableCell className="font-semibold">{emp.firstName} {emp.lastName}</TableCell>
                        <TableCell className="text-muted-foreground font-mono text-xs">{emp.employeeId}</TableCell>
                        <TableCell>{emp.designation ?? '—'}</TableCell>
                        <TableCell>{emp.department ?? '—'}</TableCell>
                        <TableCell className="font-medium">${emp.baseSalary?.toLocaleString() ?? '—'}</TableCell>
                        <TableCell>
